...
getCapitalName = function( _code ) {
      ...
      try {
        ...
        console.log("No capital name found for " + getCountryName( _code ) );
...
getCapitalPic = function(_code) {
      ...
      try {
        ...
        console.log("No capital picture found for " + getCountryName( _code ) );
...
getCountryName = function(_code) {

      try {

        var n = getCountryRec( _code ).n;
      }
      catch(err) {
...
getCountryRec = function(_code) {

    return ( ghC.findOne( { c: _code } ) );
}

getFlagPicByCode = function(_code) {
    ...
    if (typeof ghImage.findOne( { cc: _code, dt: "flg" } ) !== 'undefined') {
      ...
      console.log("No flag pic found for " + getCountryName( _code ));
...
createUserOptions = function(_obj) {
    ...
    options.password = _obj.password;

    options.username = _obj.name;

    options.email = _obj.email;
