...
LessonWorldMap = function( _mapCtl ) {
    ...
    this.doCurrentMap = function() {
        ...
        var _mapLevel = this.mapCtl.mapLevel;

        var _drawLevel = this.mapCtl.drawLevel;

        var _detailLevel = this.mapCtl.detailLevel;
    ...
    this.doThisMap = function(_mapLevel, _drawLevel, _detailLevel, _continentID, _regionID, _dontIdentify) {
        ...
        this.mapCtl.mapLevel = _mapLevel;

        this.mapCtl.drawLevel = _drawLevel;
        
        this.mapCtl.detailLevel = _detailLevel;
    ...
    this.doThisMapFromTL = function( _arr ) {
        ...
        if (_arr[0]) lessonMap.mapLevel = _arr[0];

        if (_arr[1]) lessonMap.drawLevel = _arr[1];
    
        if (_arr[2]) lessonMap.detailLevel = _arr[2];

        if (_arr[3]) lessonMap.worldMap.selectedContinent = _arr[3];

        if (_arr[4]) lessonMap.worldMap.selectedRegion = _arr[4];

        lessonMap.worldMap.doMap(lessonMap.mapLevel, lessonMap.drawLevel, lessonMap.detailLevel, lessonMap.worldMap.selectedContinent);
    ...
    this.labelMapObject = function(_level, _code, _x, _y, _fontSize, _col) {
        ...
        Meteor.defer( function() {lessonMap.worldMap.map.addLabel(_x, _y, _name.toUpperCase(), "", _fontSize, _col, _rot, _alpha, _bold); } );
    }
...
function refreshMap() {
    Meteor.setTimeout( function() { lessonMap.finishDraw(); }, 250);
}
